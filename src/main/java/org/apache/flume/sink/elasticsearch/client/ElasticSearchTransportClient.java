...
public class ElasticSearchTransportClient implements ElasticSearchClient {
	...
	private void configureHostnames(String[] hostNames) {
		...
		for (int i = 0; i < hostNames.length; i++) {
			...
			try {
				...
				java.util.logging.Logger.getLogger(ElasticSearchTransportClient.class.getName()).log(Level.SEVERE, "Unknown host " + host + " has been ignored.", e);
			}
		}
	}
	...
	public void addEvent(Event event, IndexNameBuilder indexNameBuilder, String indexType, long ttlMs)
		...
		/*
		 * if (ttlMs > 0) { indexRequestBuilder.setTTL(ttlMs); }
		 */
	 ...
	 * Arrays.toString(serverAddresses)); Settings settings =
	 * ImmutableSettings.settingsBuilder() .put("cluster.name",
	 * clusterName).build();
	 * 
	 * TransportClient transportClient = new TransportClient(settings); for
	 * (InetSocketTransportAddress host : serverAddresses) {
	 * transportClient.addTransportAddress(host); } if (client != null) {
	 * client.close(); } client = transportClient; }
	 */
	private void openClient(String clusterName) {
	...
	private void openLocalDiscoveryClient() {
		...
		/*
		 * Node node = NodeBuilder.nodeBuilder().client(true).local(true).node(); if
		 * (client != null) { client.close(); } client = node.client();
		 */
	}
